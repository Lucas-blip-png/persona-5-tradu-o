...
Comandos:
  ...
  assist    Gera rascunhos via IA (opcional; requer ANTHROPIC_API_KEY).
...
def cmd_assist(args: argparse.Namespace) -> int:
    ...
    if not assist.available():
        print(
            "IA indisponivel. Instale com: pip install -e \".[assist]\" e defina "
            "ANTHROPIC_API_KEY. (O fluxo manual continua funcionando.)"
        )
        return 1
